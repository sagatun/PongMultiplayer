...
public class Ball {
    ...
    public Ball() {
        ...
        position = new Vector2(PongMultiplayer.WIDTH/2, PongMultiplayer.HEIGHT/2);
        boundsBall = new Rectangle(position.x, position.y, ball.getWidth(), ball.getHeight());

        sw = PongMultiplayer.WIDTH;sh = PongMultiplayer.HEIGHT;
    ...
    public void update(float dt){
        if(position.x > sw - (bw - bw/2)) {
            speedX = -speedX;
            PongMultiplayer.p1score++;
        }
        if(position.x < 0 - bw/2){
            speedX = -speedX;
            PongMultiplayer.p2score++;
        }
